...
// 1. BNSS Chapter 1 का सम्पूर्ण Bare Act डेटा (100% Govt PDF के अनुसार)
const bnssData = [
  {
...
// 2. डेटा को प्रोजेक्ट में सेव करें
fs.writeFileSync('bnss_data.json', JSON.stringify(bnssData, null, 2), 'utf8');

// 3. प्रोजेक्ट में BNSS पेज फाइल खोजकर ऑटो-अपडेट करें
function findFile(dir, fileName) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      if (res) return res;
    } else if (file.toLowerCase().includes('bnss') && (file.endsWith('.jsx') || file.endsWith('.tsx') || file.endsWith('.js'))) {
      return fullPath;
    }
  }
  return null;
...
const targetFile = findFile(process.cwd(), 'bnss');
console.log("✅ BNSS Data JSON Generated Successfully.");
if (targetFile) {
  console.log("🎯 Found BNSS component at:", targetFile);
} else {
  console.log("ℹ️ Ensure your BNSS React page maps over the 'bare_text' property on accordion click.");
}
